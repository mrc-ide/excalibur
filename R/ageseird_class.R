...
#' An S4 class to represent an age-structured SEIRD epidemic model
#'
#' A class to tell the generic functions to apply SEIRD specific methods.
#' Inherits form agesirdModel to give it access to the SIRD calculate downstream
#' nodes.
#'
...
methods::setClass(
  "ageseirdModel",
  contains = "agesirdModel"
)


#' Produce an age structured SEIRD with time-varying Beta values
#'
#' First section of the function is a wrapper for the internal generator
#' function for the class ageseirdModel. The second part setups up the Odin model in
...
#' @return A object of class ageseirdModel with odin model code for the same model,
...
setAgeSIRD <- function(N, Betas, Lambda, Gamma, ProbOfDeath, I0, changeTimes = NULL){
  ...
  else if(any(c(Betas, Gamma, Lambda, N, I0, changeTimes) < 0)){
