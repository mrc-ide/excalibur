...
```{=latex}
\begin{equation}
R(t)=\frac{(D(t)-D(0))\times\gamma}{\alpha} + R(0). 
\end{equation}
```
...
```{=latex}
\begin{equation}
S(t) = S(0)\times \exp(\frac{(\mathrm{\textit{DR}}(0)-\mathrm{\textit{DR}}(t))\times\beta}{(N\times(\alpha+\gamma))}).
\end{equation}
```
Then expanded to the time-varying $\beta$ scenario,
```{=latex}
\begin{equation}
S(t)=S(0)\times\exp((\sum_{i=0}^n(\mathrm{\textit{DR}}(t_{c,i})-\mathrm{\textit{DR}}(t_{c,i+1}))\times\beta_i)/(N\times(\alpha + \gamma)))
\end{equation}
```
