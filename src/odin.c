...
#endif
typedef struct ageseirdGenerator_internal {
  double Alpha;
  double *Beta;
  double *Betas;
  double *changeTimes;
...
ageseirdGenerator_internal* ageseirdGenerator_get_internal(SEXP internal_p, int closed_error);
static void ageseirdGenerator_finalise(SEXP internal_p);
SEXP ageseirdGenerator_create(SEXP user);
void ageseirdGenerator_initmod_desolve(void(* odeparms) (int *, double *));
SEXP ageseirdGenerator_contents(SEXP internal_p);
SEXP ageseirdGenerator_set_user(SEXP internal_p, SEXP user);
SEXP ageseirdGenerator_set_initial(SEXP internal_p, SEXP t_ptr, SEXP state_ptr, SEXP ageseirdGenerator_use_dde_ptr);
SEXP ageseirdGenerator_metadata(SEXP internal_p);
SEXP ageseirdGenerator_initial_conditions(SEXP internal_p, SEXP t_ptr);
void ageseirdGenerator_rhs(ageseirdGenerator_internal* internal, double t, double * state, double * dstatedt, double * output);
void ageseirdGenerator_rhs_dde(size_t neq, double t, double * state, double * dstatedt, void * internal);
void ageseirdGenerator_rhs_desolve(int * neq, double * t, double * state, double * dstatedt, double * output, int * np);
void ageseirdGenerator_output_dde(size_t n_eq, double t, double * state, size_t n_output, double * output, void * internal_p);
SEXP ageseirdGenerator_rhs_r(SEXP internal_p, SEXP t, SEXP state);
...
ageseirdGenerator_internal* ageseirdGenerator_get_internal(SEXP internal_p, int closed_error) {
  ageseirdGenerator_internal *internal = NULL;
  if (TYPEOF(internal_p) != EXTPTRSXP) {
    Rf_error("Expected an external pointer");
  }
  internal = (ageseirdGenerator_internal*) R_ExternalPtrAddr(internal_p);
  if (!internal && closed_error) {
    Rf_error("Pointer has been invalidated");
  }
  return internal;
}
void ageseirdGenerator_finalise(SEXP internal_p) {
  ageseirdGenerator_internal *internal = ageseirdGenerator_get_internal(internal_p, 0);
  if (internal_p) {
...
SEXP ageseirdGenerator_create(SEXP user) {
  ageseirdGenerator_internal *internal = (ageseirdGenerator_internal*) Calloc(1, ageseirdGenerator_internal);
  internal->Beta = NULL;
  internal->Betas = NULL;
  ...
  R_RegisterCFinalizer(ptr, ageseirdGenerator_finalise);
  UNPROTECT(1);
  return ptr;
}
static ageseirdGenerator_internal *ageseirdGenerator_internal_ds;
void ageseirdGenerator_initmod_desolve(void(* odeparms) (int *, double *)) {
  ...
  ageseirdGenerator_internal_ds = ageseirdGenerator_get_internal(get_desolve_gparms(), 1);
}
SEXP ageseirdGenerator_contents(SEXP internal_p) {
  ageseirdGenerator_internal *internal = ageseirdGenerator_get_internal(internal_p, 1);
  SEXP contents = PROTECT(allocVector(VECSXP, 45));
  ...
  SET_VECTOR_ELT(contents, 17, ScalarInteger(internal->dim_E0));
  SET_VECTOR_ELT(contents, 18, ScalarInteger(internal->dim_I));
  SET_VECTOR_ELT(contents, 19, ScalarInteger(internal->dim_I0));
  SET_VECTOR_ELT(contents, 20, ScalarInteger(internal->dim_infectiousWeight));
  SET_VECTOR_ELT(contents, 21, ScalarInteger(internal->dim_infectiousWeight_1));
  SET_VECTOR_ELT(contents, 22, ScalarInteger(internal->dim_infectiousWeight_2));
  SET_VECTOR_ELT(contents, 23, ScalarInteger(internal->dim_N));
  SET_VECTOR_ELT(contents, 24, ScalarInteger(internal->dim_R));
  SET_VECTOR_ELT(contents, 25, ScalarInteger(internal->dim_R0));
  SET_VECTOR_ELT(contents, 26, ScalarInteger(internal->dim_S));
  SET_VECTOR_ELT(contents, 27, ScalarInteger(internal->dim_S0));
  ...
  SET_VECTOR_ELT(contents, 31, infectiousWeight);
  ...
  SET_VECTOR_ELT(contents, 32, initial_D);
  ...
  SET_VECTOR_ELT(contents, 33, initial_E);
  ...
  SET_VECTOR_ELT(contents, 34, initial_I);
  ...
  SET_VECTOR_ELT(contents, 35, initial_R);
  ...
  SET_VECTOR_ELT(contents, 38, ScalarReal(internal->Lambda));
  SEXP N = PROTECT(allocVector(REALSXP, internal->dim_N));
  memcpy(REAL(N), internal->N, internal->dim_N * sizeof(double));
  SET_VECTOR_ELT(contents, 39, N);
  SET_VECTOR_ELT(contents, 40, ScalarInteger(internal->offset_variable_D));
  SET_VECTOR_ELT(contents, 41, ScalarInteger(internal->offset_variable_I));
  SET_VECTOR_ELT(contents, 42, ScalarInteger(internal->offset_variable_R));
  SEXP R0 = PROTECT(allocVector(REALSXP, internal->dim_R0));
  memcpy(REAL(R0), internal->R0, internal->dim_R0 * sizeof(double));
  SET_VECTOR_ELT(contents, 43, R0);
  SEXP S0 = PROTECT(allocVector(REALSXP, internal->dim_S0));
  memcpy(REAL(S0), internal->S0, internal->dim_S0 * sizeof(double));
  SET_VECTOR_ELT(contents, 44, S0);
  SEXP nms = PROTECT(allocVector(STRSXP, 45));
  ...
  SET_STRING_ELT(nms, 17, mkChar("dim_E0"));
  SET_STRING_ELT(nms, 18, mkChar("dim_I"));
  SET_STRING_ELT(nms, 19, mkChar("dim_I0"));
  SET_STRING_ELT(nms, 20, mkChar("dim_infectiousWeight"));
  SET_STRING_ELT(nms, 21, mkChar("dim_infectiousWeight_1"));
  SET_STRING_ELT(nms, 22, mkChar("dim_infectiousWeight_2"));
  SET_STRING_ELT(nms, 23, mkChar("dim_N"));
  SET_STRING_ELT(nms, 24, mkChar("dim_R"));
  SET_STRING_ELT(nms, 25, mkChar("dim_R0"));
  SET_STRING_ELT(nms, 26, mkChar("dim_S"));
  SET_STRING_ELT(nms, 27, mkChar("dim_S0"));
  SET_STRING_ELT(nms, 28, mkChar("E0"));
  SET_STRING_ELT(nms, 29, mkChar("Gamma"));
  SET_STRING_ELT(nms, 30, mkChar("I0"));
  SET_STRING_ELT(nms, 31, mkChar("infectiousWeight"));
  SET_STRING_ELT(nms, 32, mkChar("initial_D"));
  SET_STRING_ELT(nms, 33, mkChar("initial_E"));
  SET_STRING_ELT(nms, 34, mkChar("initial_I"));
  SET_STRING_ELT(nms, 35, mkChar("initial_R"));
  SET_STRING_ELT(nms, 36, mkChar("initial_S"));
  SET_STRING_ELT(nms, 37, mkChar("interpolate_Beta"));
  SET_STRING_ELT(nms, 38, mkChar("Lambda"));
  SET_STRING_ELT(nms, 39, mkChar("N"));
  SET_STRING_ELT(nms, 40, mkChar("offset_variable_D"));
  SET_STRING_ELT(nms, 41, mkChar("offset_variable_I"));
  SET_STRING_ELT(nms, 42, mkChar("offset_variable_R"));
  SET_STRING_ELT(nms, 43, mkChar("R0"));
  SET_STRING_ELT(nms, 44, mkChar("S0"));
...
SEXP ageseirdGenerator_set_user(SEXP internal_p, SEXP user) {
  ...
  internal->dim_E = internal->dim_S0;
  internal->dim_E0 = internal->dim_S0;
...
SEXP ageseirdGenerator_initial_conditions(SEXP internal_p, SEXP t_ptr) {
  ageseirdGenerator_internal *internal = ageseirdGenerator_get_internal(internal_p, 1);
  SEXP r_state = PROTECT(allocVector(REALSXP, internal->dim_D + internal->dim_E + internal->dim_I + internal->dim_R + internal->dim_S));
...
void ageseirdGenerator_rhs(ageseirdGenerator_internal* internal, double t, double * state, double * dstatedt, double * output) {
  ...
  for (int i = 1; i <= internal->dim_I; ++i) {
    dstatedt[internal->offset_variable_I + i - 1] = E[i - 1] * internal->Lambda - I[i - 1] * internal->Gamma - I[i - 1] * internal->Alpha;
  }
  for (int i = 1; i <= internal->dim_R; ++i) {
    dstatedt[internal->offset_variable_R + i - 1] = I[i - 1] * internal->Gamma;
  }
  for (int i = 1; i <= internal->dim_N; ++i) {
    internal->N[i - 1] = S[i - 1] + E[i - 1] + I[i - 1] + R[i - 1] + D[i - 1];
  }
  ...
  for (int i = 1; i <= internal->dim_E; ++i) {
    dstatedt[internal->dim_S + i - 1] = S[i - 1] * odin_sum2(internal->infectiousWeight, 0, internal->dim_infectiousWeight_1, i - 1, i, internal->dim_infectiousWeight_1) - E[i - 1] * internal->Lambda;
  }
  for (int i = 1; i <= internal->dim_S; ++i) {
...
void ageseirdGenerator_rhs_dde(size_t neq, double t, double * state, double * dstatedt, void * internal) {
  ageseirdGenerator_rhs((ageseirdGenerator_internal*)internal, t, state, dstatedt, NULL);
}
void ageseirdGenerator_rhs_desolve(int * neq, double * t, double * state, double * dstatedt, double * output, int * np) {
  ageseirdGenerator_rhs(ageseirdGenerator_internal_ds, *t, state, dstatedt, output);
}
void ageseirdGenerator_output_dde(size_t n_eq, double t, double * state, size_t n_output, double * output, void * internal_p) {
  ageseirdGenerator_internal *internal = (ageseirdGenerator_internal*) internal_p;
  cinterpolate_eval(t, internal->interpolate_Beta, internal->Beta);
  memcpy(output + 0, internal->Beta, internal->dim_Beta * sizeof(double));
}
SEXP ageseirdGenerator_rhs_r(SEXP internal_p, SEXP t, SEXP state) {
  ...
  ageseirdGenerator_rhs(internal, scalar_real(t, "t"), REAL(state), REAL(dstatedt), output);
  UNPROTECT(1);
  return dstatedt;
}
agesirdGenerator_internal* agesirdGenerator_get_internal(SEXP internal_p, int closed_error) {
